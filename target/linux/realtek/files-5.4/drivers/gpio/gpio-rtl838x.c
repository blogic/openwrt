...
static int rtl838x_direction_input(struct gpio_chip *gc, unsigned int offset)
{
	...
	if (offset < 32) {
		rtl83xx_w32_mask(1 << offset, 0, RTL838X_GPIO_PABC_DIR);
		return 0;
	}
...
static int rtl838x_direction_output(struct gpio_chip *gc, unsigned int offset, int value)
{
	pr_debug("%s: %d\n", __func__, offset);
	if (offset < 32)
		rtl83xx_w32_mask(0, 1 << offset, RTL838X_GPIO_PABC_DIR);
...
void rtl838x_gpio_set(struct gpio_chip *gc, unsigned int offset, int value)
{
	...
	if (offset < 32) {
		if (value)
			rtl83xx_w32_mask(0, 1 << offset, RTL838X_GPIO_PABC_DATA);
		else
			rtl83xx_w32_mask(1 << offset, 0, RTL838X_GPIO_PABC_DATA);
	}
