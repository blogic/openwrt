...
utpl -m ubus -m uci -m fs -E capab=/etc/usync/capabilities.json -E cfg=$1 -i /usr/share/usync/usync.tpl > /tmp/usync.uci
...
cp /rom/etc/config/usteer /tmp/config-shadow
cp /rom/etc/config/system /tmp/config-shadow
